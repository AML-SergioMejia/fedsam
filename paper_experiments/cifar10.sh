...
function run_adabest() {
  ...
  alpha="$1"
  python main.py -api ${wandb_api_key} -dataset cifar10 --num-rounds 1000 --eval-every 50 --batch-size 100 --num-epochs 1 --clients-per-round 10 -model cnn -lr 0.1 --weight-decay 0 -device cuda:0 -algorithm adabest --client-algorithm adabest --server-opt sgd --num-workers 0 --where-loading init -alpha ${alpha} -beta 0.95 -mu 0.02
}
