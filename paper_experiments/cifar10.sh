...
function run_fedavg() {
  echo "############################################## Running FedAvg ##############################################"
  alpha="$1"
  python main.py -api $wandb_api_key -dataset cifar10 --num-rounds 500 --eval-every 100 --batch-size 128 --num-epochs 1 --clients-per-round 5 -model cnn -lr 0.01 --weight-decay 0.0004 -device cuda:0 -algorithm fedopt --server-lr 1 --server-opt sgd --num-workers 0 --where-loading init -alpha ${alpha}
}

function run_fedavg_with_swa() {
  echo "############################################## Running FedAvg with SWA ##############################################"
  alpha="$1"
  python main.py -api $wandb_api_key -dataset cifar10 --num-rounds 500 --eval-every 100 --batch-size 128 --num-epochs 1 --clients-per-round 5 -model cnn -lr 0.01 --weight-decay 0.0004 -device cuda:0 -algorithm fedopt --server-lr 1 --server-opt sgd --num-workers 0 --where-loading init -alpha ${alpha} -swa --swa-c 10 --swa-start 7500
}

function run_fedsam() {
  echo "############################################## Running FedSAM ##############################################"
  alpha="$1"
  python main.py -api $wandb_api_key -dataset cifar10 --num-rounds 500 --eval-every 100 --batch-size 128 --num-epochs 1 --clients-per-round 5 -model cnn -lr 0.01 --weight-decay 0.0004 -device cuda:0 -algorithm fedopt --server-opt sgd --server-lr 1 --num-workers 0 --where-loading init -alpha ${alpha} --client-algorithm sam -rho 0.1 -eta 0
}
